...
import com.ludashi.benchmark.service.amazon.AmazonConstants
import com.ludashi.benchmark.service.amazon.AmazonService
import com.ludashi.benchmark.ui.component.dialog.AmazonInstallDialog
...
import com.ludashi.benchmark.utils.DateTimeUtils
...
class AmazonAppScreen : BaseAppScreen() {
    ...
    override fun getGameDisplayInfo(
        ...
        androidx.compose.runtime.DisposableEffect(productId) {
            val listener: (com.ludashi.benchmark.events.AndroidEvent.LibraryInstallStatusChanged) -> Unit = { event ->
                if (event.appId == libraryItem.gameId) {
                    Timber.tag(TAG).d("[REFRESH] Install status changed for $productId — refreshing game info")
                    refreshKey++
                }
            }
            com.ludashi.benchmark.PluviaApp.events.on<com.ludashi.benchmark.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(listener)
            onDispose {
                com.ludashi.benchmark.PluviaApp.events.off<com.ludashi.benchmark.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(listener)
            }
        }
...
override fun isInstalled(context: Context, libraryItem: LibraryItem): Boolean =
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val downloadSize = if (downloadBytes > 0L)
                    com.ludashi.benchmark.utils.StorageUtils.formatBinarySize(downloadBytes)
                else "Unknown"
                val installSize = if (installBytes > 0L)
                    com.ludashi.benchmark.utils.StorageUtils.formatBinarySize(installBytes)
                else "Unknown"

                val installDir = AmazonConstants.getGameInstallPath(context, game?.title ?: libraryItem.name)
                val availableBytes = com.ludashi.benchmark.utils.StorageUtils.getAvailableSpace(AmazonConstants.defaultAmazonGamesPath(context))
                val availableSpace = com.ludashi.benchmark.utils.StorageUtils.formatBinarySize(availableBytes)
    ...
    override fun observeGameState(
        ...
        var currentDownloadInfo: com.ludashi.benchmark.data.DownloadInfo? = null
        ...
        disposables += { com.ludashi.benchmark.PluviaApp.events.off<com.ludashi.benchmark.events.AndroidEvent.DownloadStatusChanged, Unit>(downloadStatusListener) }
        ...
        disposables += { com.ludashi.benchmark.PluviaApp.events.off<com.ludashi.benchmark.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(installListener) }
