...
class EpicManager @Inject constructor(
    ...
    suspend fun fetchManifestSizes(context: Context, appId: Int): ManifestSizes = withContext(Dispatchers.IO) {
        try {
            ...
            val manifest = com.ludashi.benchmark.service.epic.manifest.EpicManifest.readAll(manifestData.manifestBytes)

            // Calculate install size
            val installSize = manifest.fileManifestList?.elements?.sumOf { it.fileSize } ?: 0L
            val downloadSize = com.ludashi.benchmark.service.epic.manifest.ManifestUtils.getTotalDownloadSize(manifest)
            Timber.tag("Epic").d(
