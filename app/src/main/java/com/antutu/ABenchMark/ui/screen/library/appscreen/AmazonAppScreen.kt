...
import com.antutu.ABenchMark.service.amazon.AmazonConstants
import com.antutu.ABenchMark.service.amazon.AmazonService
import com.antutu.ABenchMark.ui.component.dialog.AmazonInstallDialog
...
import com.antutu.ABenchMark.utils.DateTimeUtils
...
class AmazonAppScreen : BaseAppScreen() {
    ...
    override fun getGameDisplayInfo(
        ...
        androidx.compose.runtime.DisposableEffect(productId) {
            val listener: (com.antutu.ABenchMark.events.AndroidEvent.LibraryInstallStatusChanged) -> Unit = { event ->
                if (event.appId == libraryItem.gameId) {
                    Timber.tag(TAG).d("[REFRESH] Install status changed for $productId — refreshing game info")
                    refreshKey++
                }
            }
            com.antutu.ABenchMark.PluviaApp.events.on<com.antutu.ABenchMark.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(listener)
            onDispose {
                com.antutu.ABenchMark.PluviaApp.events.off<com.antutu.ABenchMark.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(listener)
            }
        }
...
override fun isInstalled(context: Context, libraryItem: LibraryItem): Boolean =
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val downloadSize = if (downloadBytes > 0L)
                    com.antutu.ABenchMark.utils.StorageUtils.formatBinarySize(downloadBytes)
                else "Unknown"
                val installSize = if (installBytes > 0L)
                    com.antutu.ABenchMark.utils.StorageUtils.formatBinarySize(installBytes)
                else "Unknown"

                val installDir = AmazonConstants.getGameInstallPath(context, game?.title ?: libraryItem.name)
                val availableBytes = com.antutu.ABenchMark.utils.StorageUtils.getAvailableSpace(AmazonConstants.defaultAmazonGamesPath(context))
                val availableSpace = com.antutu.ABenchMark.utils.StorageUtils.formatBinarySize(availableBytes)
    ...
    override fun observeGameState(
        ...
        var currentDownloadInfo: com.antutu.ABenchMark.data.DownloadInfo? = null
        ...
        disposables += { com.antutu.ABenchMark.PluviaApp.events.off<com.antutu.ABenchMark.events.AndroidEvent.DownloadStatusChanged, Unit>(downloadStatusListener) }
        ...
        disposables += { com.antutu.ABenchMark.PluviaApp.events.off<com.antutu.ABenchMark.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(installListener) }
