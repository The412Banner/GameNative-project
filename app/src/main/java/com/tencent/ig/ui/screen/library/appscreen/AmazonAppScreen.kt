...
import com.tencent.ig.service.amazon.AmazonConstants
import com.tencent.ig.service.amazon.AmazonService
import com.tencent.ig.ui.component.dialog.AmazonInstallDialog
...
import com.tencent.ig.utils.DateTimeUtils
...
class AmazonAppScreen : BaseAppScreen() {
    ...
    override fun getGameDisplayInfo(
        ...
        androidx.compose.runtime.DisposableEffect(productId) {
            val listener: (com.tencent.ig.events.AndroidEvent.LibraryInstallStatusChanged) -> Unit = { event ->
                if (event.appId == libraryItem.gameId) {
                    Timber.tag(TAG).d("[REFRESH] Install status changed for $productId — refreshing game info")
                    refreshKey++
                }
            }
            com.tencent.ig.PluviaApp.events.on<com.tencent.ig.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(listener)
            onDispose {
                com.tencent.ig.PluviaApp.events.off<com.tencent.ig.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(listener)
            }
        }
...
override fun isInstalled(context: Context, libraryItem: LibraryItem): Boolean =
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val downloadSize = if (downloadBytes > 0L)
                    com.tencent.ig.utils.StorageUtils.formatBinarySize(downloadBytes)
                else "Unknown"
                val installSize = if (installBytes > 0L)
                    com.tencent.ig.utils.StorageUtils.formatBinarySize(installBytes)
                else "Unknown"

                val installDir = AmazonConstants.getGameInstallPath(context, game?.title ?: libraryItem.name)
                val availableBytes = com.tencent.ig.utils.StorageUtils.getAvailableSpace(AmazonConstants.defaultAmazonGamesPath(context))
                val availableSpace = com.tencent.ig.utils.StorageUtils.formatBinarySize(availableBytes)
    ...
    override fun observeGameState(
        ...
        var currentDownloadInfo: com.tencent.ig.data.DownloadInfo? = null
        ...
        disposables += { com.tencent.ig.PluviaApp.events.off<com.tencent.ig.events.AndroidEvent.DownloadStatusChanged, Unit>(downloadStatusListener) }
        ...
        disposables += { com.tencent.ig.PluviaApp.events.off<com.tencent.ig.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(installListener) }
