...
import com.mihoyo.genshinimpact.service.amazon.AmazonConstants
import com.mihoyo.genshinimpact.service.amazon.AmazonService
import com.mihoyo.genshinimpact.ui.component.dialog.AmazonInstallDialog
...
import com.mihoyo.genshinimpact.utils.DateTimeUtils
...
class AmazonAppScreen : BaseAppScreen() {
    ...
    override fun getGameDisplayInfo(
        ...
        androidx.compose.runtime.DisposableEffect(productId) {
            val listener: (com.mihoyo.genshinimpact.events.AndroidEvent.LibraryInstallStatusChanged) -> Unit = { event ->
                if (event.appId == libraryItem.gameId) {
                    Timber.tag(TAG).d("[REFRESH] Install status changed for $productId — refreshing game info")
                    refreshKey++
                }
            }
            com.mihoyo.genshinimpact.PluviaApp.events.on<com.mihoyo.genshinimpact.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(listener)
            onDispose {
                com.mihoyo.genshinimpact.PluviaApp.events.off<com.mihoyo.genshinimpact.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(listener)
            }
        }
...
override fun isInstalled(context: Context, libraryItem: LibraryItem): Boolean =
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val downloadSize = if (downloadBytes > 0L)
                    com.mihoyo.genshinimpact.utils.StorageUtils.formatBinarySize(downloadBytes)
                else "Unknown"
                val installSize = if (installBytes > 0L)
                    com.mihoyo.genshinimpact.utils.StorageUtils.formatBinarySize(installBytes)
                else "Unknown"

                val installDir = AmazonConstants.getGameInstallPath(context, game?.title ?: libraryItem.name)
                val availableBytes = com.mihoyo.genshinimpact.utils.StorageUtils.getAvailableSpace(AmazonConstants.defaultAmazonGamesPath(context))
                val availableSpace = com.mihoyo.genshinimpact.utils.StorageUtils.formatBinarySize(availableBytes)
    ...
    override fun observeGameState(
        ...
        var currentDownloadInfo: com.mihoyo.genshinimpact.data.DownloadInfo? = null
        ...
        disposables += { com.mihoyo.genshinimpact.PluviaApp.events.off<com.mihoyo.genshinimpact.events.AndroidEvent.DownloadStatusChanged, Unit>(downloadStatusListener) }
        ...
        disposables += { com.mihoyo.genshinimpact.PluviaApp.events.off<com.mihoyo.genshinimpact.events.AndroidEvent.LibraryInstallStatusChanged, Unit>(installListener) }
